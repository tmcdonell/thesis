...
extern "C" __global__ void generate(const Int64 shOut_1, const Int64 shOut_0, float* __restrict__ arrOut_1, float* __restrict__ arrOut_0)
{
...
arrOut_1[ix] = 0.0f;
arrOut_0[ix] = cosf(15.0f * (-50.0f + (float) sh0) / 100.0f) * cosf(15.0f * (-50.0f + (float) sh1) / 100.0f) / 5.0f;
}
}
