...
parseArgs argv = do
  ...
  let backends :: [OptDescr (Options -> IO Options)]
      ...
      describe f msg    = msg P.++ " (" P.++ show (get f defaultOptions) P.++ ")"
      ...
      helpMsg errs      = concat errs
        P.++ usageInfo basicHeader                    options
        P.++ usageInfo "\nGeneric criterion options:" Criterion.defaultOptions

      fancyHeader :: Options -> String
      fancyHeader opts = unlines (header : table P.++ lines basicFooter)
        where
          ...
          paste x y z         = "  " P.++ x P.++ "  " P.++ y P.++ "  " P.++ z
          sameLen xs          = flushLeft ((P.maximum . P.map P.length) xs) xs
          flushLeft n xs      = [ P.take n (x P.++ repeat ' ') | x <- xs ]
          --
