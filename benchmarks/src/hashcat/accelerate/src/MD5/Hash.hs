...
-- The dictionary of known passwords to hash.
--
-- The input dictionary of words to hash is of dimension (Z :. 16 :. n) when in
-- column format, or (Z :. n :. 16) when in row-major format.
--
-- TODO: The alignment should be baked in as part of the type of dictionary.
--
data Align      = R | C
type Dictionary = Array DIM2 Word32


-- An MD5 round processes 512 bits of the input, as 16 x 32-bit values.
--
...
md5 :: Align -> Acc Dictionary -> Acc (Vector MD5)
md5 align dict
  = let Z :. r :. c     = unlift $ A.shape dict
        n               = case align of
                            C -> c
                            R -> r
    in
    A.generate (index1 n) (md5Round align dict)


-- A single round of MD5. For inputs longer than 512-bits we would simply
-- reapply this until we run out of input data to hash.
--
md5Round :: Align -> Acc Dictionary -> Exp DIM1 -> Exp MD5
md5Round align dict (unindex1 -> word)
  = lift
  ...
  where
    ...
    getWord32le (constant -> i) =
      case align of
        C -> dict ! index2 i word
        R -> dict ! index2 word i
