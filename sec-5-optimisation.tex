...
\end{lstlisting}
%
This extended environment describes how to connect the environment of the
original term which we are currently deforesting (\texttt{aenv}), with the new
...
% \textcolor{red}{These previous bullet points can be integrated as actual text.
...
The strategy then is to manipulate the core, nameless de Bruijn\index{de Bruijn}
...
substitution machinery developed earlier (\S\ref{sec:substitution}). 
%
\begin{lstlisting}[style=haskell]
compose :: Elt c
...
\end{lstlisting}
%
Annoyingly, the second case is required simply to suppress a compilation
warning; GHC's type checker can not determine that the type of the function
prohibits any valid term except that caught by the first pattern.
...
want to combine terms --- whether that be scalar function composition or as
...
weaken a term by a set of indices specified by an external witness.
% 
\begin{lstlisting}[style=haskell]
sink :: Extend env env'
     -> Idx env  t
