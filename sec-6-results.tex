...
simulation is shown in Listing~\ref{lst:nbody}, where \code{accel} calculates
the acceleration between two particles, and \code{(.+.)} component-wise sums the
...
results. We leave this to future work, for example, in the style of data flow
...
The \code{radixPass} function sorts the vector \code{v} based on the value of
bit \code{k}; moving elements with a zero at that bit to the beginning of the
