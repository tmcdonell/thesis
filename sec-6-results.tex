...
\note{tk: using shared memory. This is difficult because our stencils lack
significant structure: i.e. they can be quite sparse, so reading all
elements into shared memory (wlog, in itself tricky!) might be a waste. We avoid
this issue and just use the texture cache. It might be worthwhile to specialise
for the 2D texture cache (and extra addressing modes?).}
...
visible with smaller image sizes. Also shown are the run times for just the
first six data parallel phases, which exhibit linear speedup.

Neglecting the final phase, note that the data parallel phase is still slightly
slower than in OpenCV. As discussed previously
(\S\ref{sec:stencil_bounds_checking}) this is because the stencil kernels in
...
\marginnote{tk: this sort of went in a different direction?}
...
the coefficients required to compute the derivative at each point. As with all
shortcut fusion methods, the system presented here does not combine multiple
passes over the same array into a single traversal that computes multiple
results. We leave this for future work, for example, in the style of data flow
fusion~\cite{Lippmeier:2013vz}.
\note{tk: simpler, more motivating example?}
