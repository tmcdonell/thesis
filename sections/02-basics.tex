...
In a GPU, the large caches and complex instruction scheduling logic of the CPU
that increase single-threaded performance are instead replaced with many
additional arithmetic units.
...
\item The input array is divided into chunks of size $B$ elements and
    ...
    @block_results[i]@.\footnote{We note that computation of the final value
    requires that elements are processed in the correct left-to-right or
    right-to-left order. For simplicity, the current implementation reuses the
    block scan kernel implemented here, and therefore, as described, all but the
    final result of the scan is discarded. Replacing this step with a
    directional reduce operation would likely improve performance, but this
...
% Although the current implementation discards all but the last element of each of
% the sub-arrays in step 1, this behaviour may not be necessary. A critical
% performance evaluation of Accelerate's scan implementation would be valuable
% future work.
...
twice: once when computing the partial block results (step 1), and a second time
when computing the final scanned array (step 3). If the input does not refer to
manifest array data, but instead an array in a symbolic form resulting from
array fusion\fusion{} (\S\ref{sec:array_fusion}), then the elements of the
symbolic array --- which might contain arbitrarily expensive computations ---
will be computed twice also.
...
% the user has no control as to whether the input array is manifest or symbolic,
% and no special action is taken on the part of the backend in its current
% implementation, although both alternatives are possible avenues of future
% work.
...
\footnote{One might further categorise \emph{combinations} of shallow and deep
embeddings~\cite{Svenningsson:2013kb}. This amounts to a shallow embedding which
targets operations in a deeply embedded core language, and is equivalent to, for
example, the definition of \footcode{filter} (Listing~\ref{lst:filter}) being a
composition of several operations, rather than a core language primitive.}
