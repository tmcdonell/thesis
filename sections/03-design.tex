...
blocks}\index{GPU!thread block}. Thread blocks execute independently of each
other, so a GPU with a higher number of CUDA cores --- or \emph{streaming
multiprocessors}\index{GPU!streaming multiprocessor} (SM) --- will execute the
...
execute the kernel at each invocation is specified in the @<<<...>>>@ syntax.
% Moreover, Listing~\ref{lst:cuda_vecadd} demonstrates that the GPU programming
% model as exposed by CUDA is data parallel --- $N$ threads execute $N$ individual
% instances of the kernel function @VecAdd@, and each thread operates on a single
% element of each input array to create a single value in the result.
%
