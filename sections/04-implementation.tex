...
kernel notion of pinned memory as specified using \footcode{mlock()}, since DMA
requires not only the virtual address but physical address to remain constant.}
memory. Note that page-locked memory is a scarce resource and reduces the amount
of physical memory available to the operating system, so consuming too much
...
behaviour. In our case, the various collective operations in Accelerate
constitute skeletons that are parameterised by types and scalar embedded
expressions to be injected at predefined points. The skeletons for collective
operations are hand-tuned to ensure efficient global-memory access and use of
fast on-chip shared memory for intra-block communication, all of which is
necessary to ensure good performance on the \GPU~\cite{NVIDIA:2012wf}.

\subsection{Algorithmic Skeletons}

The code generator includes \CUDA\ code skeletons implementing each of the
collective operations in Accelerate in such a manner that the code generator can
instantiate each skeleton with a set of concrete parameters.
Listing~\ref{lst:map_skeleton} shows the skeleton code for the \code{map}
operation. The skeleton implements the \CUDA\ kernel~\cite{NVIDIA:2012wf} that
encodes the behaviour of each thread for this computational pattern, fixing the
parallel algorithmic structure of the operation, and contains place holders for
the element types of the input and output arrays, as well as the function to
apply at each element.

% The \code{map()} function in Listing~\ref{lst:map_skeleton} uses the
% quasiquoting facility of the \texttt{language-c-quote}
% library~\cite{Mainland:2007bl}. Quasiquotation allows a program to use the
% \emph{concrete syntax} of a language to construct program fragments in that
% language. Of particular importance, however, is that 

The \code{map} function in Listing~\ref{lst:map_skeleton} uses
quasiquotation~\cite{Mainland:2007bl} in order to construct the \emph{abstract}
syntax representing the program fragment using the \emph{concrete} syntax of the
target language, in this case \CUDA\@. Since quasiquoters are run at Haskell
compile time, any parsing errors or errors in generated Haskell abstract syntax
are therefore also caught at compile time, ensuring that the skeleton which is
instantiated at runtime will be syntactically correct.
% . In contrast to generating concrete
% syntax as strings, this statically ensures that all generated \CUDA\ code is
% syntactically correct (assuming the abstract syntax is pretty-printed as valid
% concrete syntax).
Furthermore, an expression such as \code{\$exp:(threadIdx dev)} is an
\emph{antiquotation} that causes, at program runtime, the value of evaluating
the Haskell function \code{threadIdx} with input \code{dev} to be spliced into
the abstract syntax tree for the quasiquoted \CUDA\ \code{map} skeleton.

\begin{lstlisting}[style=haskell,
    float,
    name=map_skeleton,
    label=lst:map_skeleton,
    caption={CUDA skeleton for the \code{map} operation}]
[cunit|
...
%
\begin{enumerate}
\item The types of the input and output arrays \code{argIn} and \code{argOut},
    respectively.

\item The function \code{get}, which retrieves or generates the appropriate
    input array element for array index \code{ix}.

\item The function \code{f} which is applied to that element.
\end{enumerate}
...
\footnote{\url{http://hackage.haskell.org/package/language-c-quote}} furthermore
allows the \CUDA\ \AST\ can be constructed using concrete syntax.
% , which can later be pretty printed to file for external compilation
% (\S\ref{sec:external_compilation}).
The following features of code generation deserve particular attention: (1)
lambda abstractions; (2) let bindings; (3) shapes; (4) array references; and (5)
tuples.
...
Accelerate directly supports collective operations on multidimensional arrays.
In C, we represent multidimensional shapes and indices as a \code{struct}:
%
\begin{lstlisting}[style=cuda]
typedef Int32                                     Ix;
typedef void*                                     DIM0;
...
of separate program on the \GPU\@. This is inefficient, and often we want to
fuse two skeletons into a single operation, avoiding the creation and traversal
of an intermediate array. This is the task of the fusion optimisation: to
combine networks of collective operations into a single operation
(\S\ref{sec:fusion}).
