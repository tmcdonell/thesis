...
\begin{figure}
    ...
    \end{center}
    \caption[A parallel inclusive scan]{Illustration of a parallel scan, which
        is performed in three steps. In the first step, 8 thread blocks compute
        the partial scan result for each section of the array. The partial
        results are then scanned by a single thread block in the second step.
        In the final phase, the 8 thread blocks use the partial results as the
        initial element when computing the final scan result for this section of
        the array.}
    \label{fig:fast_scan}
\end{figure}
...
algorithm is depicted in Figure~\ref{fig:fast_scan}, and proceeds in three
phases:
%
\begin{enumerate}
