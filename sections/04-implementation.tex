...
code. This was seen in the \code{fst4} example, which generates significant
memory traffic if tuples are represented as \code{struct}s, even though three of
...
-> [(Bool,a)]} on the line marked \emph{(2)} determines for each term whether it
is subsequently used. Thus, when the code generated by \code{get} reads data
from the input array, it doesn't read the unused values. Consequently,
\code{fst4} only touches the array representing the first component of the
quadruple of arrays.
