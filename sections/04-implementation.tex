...
The Accelerate CUDA backend is based on the idea of
\index{skeleton}\emph{algorithmic skeletons}~\cite{Cole:1989vr}, where a
parallel program is composed from one or more parameterised skeletons, or
templates, encapsulating specific parallel behaviour. In our case, the backend
implements each of the aggregate array operations, such as @map@, by way of a
CUDA~(\S\ref{sec:cuda}) \emph{code template} that is parameterised with array
types and worker functions, such as the function to be mapped, to be injected at
predefined points.
...
the elements of three of the four arrays are never used. Alas, this does not, and
...
internal \indext{de Bruijn} representation, sharing of scalar subcomputations is
...
used in the generated scalar code, and when splicing assignments into a skeleton
template, elides statements whose results are not used. The following instance
...
using the second approach, such as def-use analysis (our implementation of
...
existing highly optimised CUDA libraries that implement, for example, for high performance
...
\indexe{nursery}, which is simply a finite mapping from array sizes to a chunk
...
otherwise fresh memory is allocated.
