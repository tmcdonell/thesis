...
represents the embedded program, which Accelerate's \CUDA backend then converts
into \CUDA C code that is compiled, loaded onto the \GPU, and executed.
...
remove some forms of dead code. This was seen in the \code{fst4} example, which
generates significant memory traffic if tuples are represented as
\code{struct}s, even though three of the four values read from memory were never
used. With the use of template meta programming to inline the definition of
scalar expressions directly into the skeleton template, the situation is greatly
improved.
...
codegenOpenFun1 dev env aenv (Lam (Body f))
  = let
        dummy        = locals "undefined_x" (undefined :: a)                        -- (1)
        Gen _ _ used = execGen $ codegenOpenExp dev aenv f (env `Push` dummy)       -- (2)
    ...
\end{lstlisting}
%
...
%
\begin{lstlisting}[style=haskell]
deadCodeElim :: HashSet C.Exp -> [C.Exp] -> [a] -> [(Bool,a)]
...
Completing our definition of \code{codegenOpenFun1}, we return the result of
...
method requires performing code generation twice for every expression.
%
...
codegenOpenFun1 dev env aenv (Lam (Body f))
  = let
    ...
    CUFun1 (deadCodeElim used dummy)
           (\xs -> evalGen $ codegenOpenExp dev aenv f (env `Push` xs))
\end{lstlisting}
...
components of the shape. Operations such as \code{toIndex} are implemented
directly in the skeleton, rather than packing the shape components into a
\code{struct} and then calling the family of overloaded functions.. This
component-wise representation is identical to the treatment of tuples, which are
discussed in the following section.


% Note we have set the type of each component of the shape in C to a 32-bit
% integer, whereas indices in Haskell are represented using \code{Int}. Depending
% on the architecture of the host Haskell compiler, this int might be either 32-
% or 64-bits. This entails additional conversions when marshalling shapes from
% Haskell, but can result in higher performance code, because \CUDA devices are
% natively 32-bit processors incur a significant performance penalty when
% executing 64-bit arithmetic.
% 
% As \CUDA supports the use of C++ features, such as templates and function
% overload, we simplify code generation by overloading functions which operate on
% indices for the various index types. That is, we have the following family of
% functions:
% %
% \begin{lstlisting}[style=cuda,mathescape]
% int  dim(DIM$n$ sh);                        // number of dimensions in a shape
...
% \end{lstlisting}
% 
% The major advantage of this approach is that the \CUDA skeletons are ad-hoc
% polymorphic; i.e., we have a single skeleton that during \emph{instantiation} is
% specialised to a particular array element type and dimensionality.
% 
% That the underlying representation as a C \code{struct} is only relevant when
% accessing shape data that was marshalled as a parameter to the kernel, in which
% case field accessors must be generated. In all other circumstances however, the
% shape can be treated as a tuple structure. Tuples are explained in further
% detail below.
