...
array with shape \code{(Z :. 2 :. n)} will use two thread blocks, no matter how
large \code{n} is.\footnote{The number of multiprocessors on a device various
between architecture generation and performance of a given card. For example a
compute 1.x device will have 32 cores per multiprocessor, a compute 2.1 device
48 cores per multiprocessor, and a compute 3.x device uses 192 cores per
multiprocessor. This generally results in a fewer number of multiprocessors for
later generation devices, even though the total core count is typically
greater.}
