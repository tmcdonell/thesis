...
function \code{f} on elements of type \code{T}, the following atomically
combines a value from the source array \code{x} with the value of the result
array \code{y}.
...
\code{map} and \code{fold}, but also array-oriented operations, such as
\code{permute} and stencil convolutions.
...
\end{lstlisting}
%
The crucial difference to vanilla Haskell is the \code{Acc} type constructor
representing \emph{embedded array-valued computations}. The types
\code{Vector e} and \code{Scalar e} represent one-dimensional and
zero-dimensional (singleton) arrays, respectively.

The expression \code{zipwith (*) xs ys} implements point-wise multiplication of
the two argument vectors, and \code{fold (+) 0} sums the resulting products up
...
computations} of type \code{Exp a} to implement a parallel reduction along the
innermost dimension of an $n$-dimensional, embedded array of type \code{Array
(sh:.Int) a}. The \emph{shape} \code{sh:.Int} consist of a polymorphic shape
\code{sh} with one added (innermost) dimension, which is missing from the shape
of the result array.
...
operations, such as \code{map}, by way of a CUDA C \emph{code template} that is
...
Listing~\ref{lst:map_skeleton} shows the skeleton code for the \code{map}
operation. The \code{[cunit|...|]} brackets enclose \CUDA C definitions
comprising this skeleton template. \CUDA uses the \code{__global__} keyword to
indicate that \code{map} is a \emph{GPU} \indexe{kernel}~\cite{NVIDIA:2012wf}
--- a single data-parallel computation launch on the \GPU by the CPU.
\emph{Antiquotations}\index{antiquotation} \code{$params:var}, \code{$exp:var},
\code{$items:var}, and \code{$stms:var} denote template parameters using a
Haskell variable \code{var} to splice \CUDA C parameters, expressions, items,
...
In order to instantiate the \code{map} skeleton, the code generator needs to
generate concrete parameters for all of the holes in the skeleton template. In
particular:
%
\begin{enumerate}
\item The function \code{f} which is applied to each individual array element;

\item The function \code{get}, which extracts the appropriate input array
    element for the array index \code{ix}; and finally

\item The types of the input and output array arguments are collected and
    spliced into \code{argIn} and \code{argOut}, respectively.

\end{enumerate}
%
The meaning of the auxiliary combinators \code{get}, \code{setOut}, \code{dce},
and \code{(.=.)} will be explained in the following sections.

As the quasiquoter \code{[cunit|...|]} is executed at Haskell compile time,
...
\end{lstlisting}
%
The \code{zipWith} and \code{fold} operations would each compile to separate
skeletons, and as a result the execution of \code{zipWith} produces an
intermediate array that the \code{fold} kernels immediately consume.

This is not what a CUDA programmer would manually implement. It is more
efficient to inline the \code{zipWith} computation into the kernel of the
\code{fold}. This strategy eliminates one GPU kernel, as well as one
...
independently (such as \code{map}), from skeletons where threads cooperate to
produce an array (such as \code{fold}). Central to this approach is a
...
\end{lstlisting}
%
Instead of generating a skeleton instance for \code{zipWith} straight away, we
represent the computation implemented by \code{zipWith} as a scalar function
...
type \code{DelayedOpenAcc}. For details of the representation see
section~\ref{sec:implementing_array_fusion}.

The crucial step in enabling producer-consumer fusion in Accelerate's CUDA
backend is in the function \code{codegenAcc}, which turns a manifest array
computation of type \code{DelayedOpenAcc aenv a} into an instantiated CUDA
kernel of type \code{CUSkeleton aenv a}:
%
\begin{lstlisting}[style=haskell]
...
\end{lstlisting}
%
Here we see that \code{mkFold}, which generates an instance of the \code{fold}
template, gets as its last argument the code generated for a delayed array
resulting from the call to \code{codegenDelayed}. The use of template meta
...
producer is equivalent to the scalar function \code{\\ix -> (xs!ix) * (ys!ix)}.
The call to \code{mkFold} receives a CUDA version of this function, which can be
inlined directly into the \code{fold} skeleton. The delayed producer function is
bound to the argument \code{getIn} in the \code{mkFold} definition given in
...
\end{lstlisting}
%
Here the \code{assert} checks whether the array index is in bounds, and is only
...
mode, and the command line flag \code{-ddebug-cc} is provided.} In this case the
embedded \code{zipWith} operation is over two vectors, so no conversion to and
from multidimensional indices was required.

In contrast to the \code{map} skeleton shown in Listing~\ref{lst:map_skeleton},
the code generated by \code{mkFold} proceeds in to parallel phases. The first
phase is a sequential \code{for} loop including the use of the \code{getIn}
...
functions, such as the mapping function for \code{map} or the combination
function for \code{fold}. Thus, a crucial part of template instantiation is
...
operator \code{(.=.)}. For example, the operator was used in lines \emph{(1)}
...
\code{const} declarations, or as a statement updating a mutable variable. The
...
Accelerate arrays of primitive type, such as \code{Float} and \code{Int}, are
...
implementation of tuples in CUDA might use arrays of \code{struct} type --- that
is, we might consider representing values of type \code{Array DIM1 (Int, Float)}
...
abstract over array element types, C \code{struct}s were used to represent
tuples of scalar values. A family of getter and setter functions were then
generated to create and consume these \code{struct}s when reading and writing
...
\end{lstlisting}
%
The function \code{unlift} turns an embedded scalar expression that yields a
...
expression. Moreover, in \code{fst4} under the old code generation strategy,
array elements are copied into a \code{struct}, only for the first element to be
extracted again and the \code{struct} to be discarded. One might hope that the
...
as a result \code{fst4} generates considerable memory traffic.

With template meta programming and the \code{Assign} type class introduced
...
into a \code{struct}, we represent it by a list of primitive values, one per
...
The \code{(.=.)} operator allows us to assign all values constituting the tuple
...
(\S\ref{sec:representing_tuples}). The function \code{pushEnv} generates a fresh
...
local declarations to be evaluated first. With the \code{Assign} type class
...
of the C skeleton. This is in contrast to the definition of the \code{Let} \AST
...
remove some forms of dead code. This was seen in the \code{fst4} example of
section~\ref{sec:representing_tuples}, which generates significant memory
traffic if tuples are represented as \code{struct}s, even though three of the
...
instance of the \code{Assign} class uses a flag that is \code{False} to indicate
...
The \code{map} skeleton of Listing~\ref{lst:map_skeleton} exploits this: when
generating code for the mapped function \code{f}, the function \code{dce :: [a]
...
\code{get} reads data from the input array, it doesn't read the unused values.
Consequently, \code{fst4} only touches the array representing the first
component of the quadruple of arrays.

To generate the function \code{dce} we need to determine which inputs to the
generated scalar function are ultimately used in the calculation. Code
generation for a function such as \code{fst4} begins by initialising the
...
Dead-code analysis then builds the function \code{dce} by testing whether each
...
general, we check the result from each step of \code{codegenOpenExp} with the
function \code{visit}, and test if the result was a scalar C variable:
%
\begin{lstlisting}[style=haskell]
...
Completing our definition of \code{codegenFun1}, we return the result of
dead-code analysis produced by \code{deadCodeElim}, together with a function
...
type \code{Array sh e}, where \code{sh} is the \emph{shape} and \emph{e} is the
...
level, the constructor \code{Z} is used to represent the shape of a rank-0
array, and the infix operator \code{(:.)} to increase the rank by adding a new
...
represented multidimensional shapes and indices as a \code{struct}.
%
% \begin{lstlisting}[style=cuda]
% typedef Int32                           Ix;
% typedef void*                           DIM0;
...
for operating on shapes and indices --- such as \code{toIndex} and
\code{fromIndex} --- so that skeletons were not specialised to a particular
dimensionality.
%
...
\code{struct}s to represent shapes. The new approach represents shapes during
...
\code{toIndex} are implemented directly in the skeleton, rather than packing the
shape components into a \code{struct} and then calling the family of overloaded
...
arguments to collective operations, such as \code{zipWith}. This restriction is
...
\code{codegenFun1} operation to generate the CUDA code for the unary function,
...
the \code{Assign} class discussed earlier.
...
namely array indexing (\code{!} and \code{!!}) and determining the \code{shape}
...
code includes the following use of \code{backpermute} to extract the values of
...
% \end{lstlisting}
%
Here the array computation \code{inds :: Acc (Array DIM1 Float)} is used in the
first and second argument of \code{backpermute}. In the code for \code{smvm},
\code{inds} is a previously let-bound variable. If instead, a collective
operation would have been used in place of \code{inds} in the scalar function
\code{\i -> index1 \$ inds!i}, we would lift it out of the scalar function and
...
add a new argument to the prototype of the \code{backpermute} skeleton, as the
generated code simply included the skeleton definition using a C pre-processor
\code{include} directive. This problem was worked around by using \emph{texture
...
consequence. Skeletons such as \code{backpermute} allow unconstrained indexing
...
\code{StateT} monad stacked over \code{IO}. This is necessary to use the Haskell
...
that \code{run} is a pure function at the level of the user-level Accelerate
...
itself --- that is, \code{Tree} is a recursively defined data type.
...
additional step of needing to unwrap the \code{Tree} constructor before exposing
the \code{Leaf} and \code{Node} constructors of the tree itself:
%
...
array programs in the CUDA backend. In addition to the function \code{run}
...
% functionality by use of the \code{run1} operation.

Following the fusion transformation, the Accelerate array program is
parameterised by a series of \code{Manifest} and \code{Delayed} nodes,
...
an important optimisation, which we provide through use of the \code{run1}
...
given some initial values \code{df0} and \code{vf0} the simulation proceeds as:
%
\begin{lstlisting}[style=haskell]
...
performed once, so we simply ensure that the data for a \code{use} operation is
...
goes. Furthermore, the memory table is wrapped in an \code{MVar}, so that it can
...
\item However there is an awkward race condition here, because at the moment
    ...
    unreachable) before \code{deRefWeak} gets to it. In this unusual case we
    throw an error, but in doing so this makes \code{arr} reachable in the
    continuation of \code{deRefWeak}, and thus ensures \code{deRefWeak} will
    always succeed. This sort of weirdness, typical of the world of weak
    pointers, is why we can not reuse the stable name \code{sa} computed above
