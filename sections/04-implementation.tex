...
massively data-parallel hardware. This chapter details the implementation of the
...
the programmer. Moreover, the Accelerate language is more expressive, supporting
multidimensional arrays, generative functions such as @replicate@, and
Accelerate computations can span multiple GPU kernels. Both Nikola and Obsidian
can only express array computations that can be implemented in a single GPU
kernel over one dimensional vectors. In both Obsidian and Nikola, algorithms
spanning multiple kernels need to be explicitly scheduled by the application
programmer and incur additional host-device data transfer overhead, which incurs
as severe performance penalty.
