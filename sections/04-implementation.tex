...
Haskell~\cite{Sheard:2002wu} to define skeletons as quoted CUDA C templates with
splices for the template parameters.
...
Haskell variable \code{var} to splice CUDA C parameters, expressions, items, and
...
\CUDA\ \AST\ to be constructed using concrete syntax. The following features of
code generation deserve particular attention: lambda abstractions; let
bindings; shapes; array references; and tuples.
...
level, the constructor \code{Z} is used to represent the shape of a rank-0 array
and, and the infix operator \code{:.} to increase the rank by adding a new
...
However, the new code generation method~\cite{CliftonEverest:2014vi} does not
require the use of \code{struct}s to represent shapes or tuples. The new
approach represents shapes by their individual components, and operations such
as \code{toIndex} are implemented directly in the skeleton, rather than packing
the shape components into a \code{struct} and then calling the family of
overloaded functions. This is component-wise representation identical to the
treatment of tuples, which are discussed in further detail below.
