...
can be safely shared. This reduces the amount of memory required to store the
...
are effectively constructing two separate \footcode{Use} nodes. Furthermore,
since GHC does not do common subexpression elimination, as it can affect the
strictness/laziness of the program and potentially introduce space leaks, these
common subexpressions are not combined, two separate values are created on the
heap, and there is no sharing to be observed.}
%
\begin{lstlisting}[style=haskell]
...
constrained to work within the evaluation of a single Accelerate computation.
Thus, localised memory management techniques such as reference counting or
syntax directed allocations within a single \code{run} invocation are
insufficient.

The key observation is that the Haskell runtime system knows when objects are no
longer required and can be garbage collected, so
finalisers~\cite{PeytonJones:2000ks} are attached to each array which deallocate
the foreign memory at the same time the Haskell heap object is garbage
collected. To be precise, since the CUDA backend uses a hash table to associate
each array on the host to the corresponding memory on the GPU, we construct a
weak hash table / memo table~\cite{PeytonJones:2000ks}, so that both entries can
deallocate the device memory and remove themselves from the hash table when the
array is garbage collected, and all finalisers can be initiated immediately if
...
% 
% 
% The first task is accomplished simply by coping data to the GPU as soon as we
% encounter the \code{Use} node in an expression. 
% 
% There are several approaches that could be used to solve the second problem of
% when to deallocate memory, and some alternative approaches are described briefly
% below (\S\ref{sec:memory_management_alternatives}). However, none of those
% approaches are able to share arrays \emph{across} computations.
% 
% 
% 
% 
% Here \code{f} is an Accelerate program that evaluates to an array \code{xs}. The
% array \code{xs} must be copied to the host after evaluating the expression to be
% made available for use in vanilla Haskell. Following this, the computation of
% the array \code{ys} involves evaluating the program of one array argument
% \code{g} using the first result \code{xs} as input, which is re-injected into
% the embedded language with \code{use}. Since \code{xs} was just copied
% \emph{from} the device after 
% 
% 
% eliminate redundant
% data transfers between separate computations.
% 
% 
% \subsection{Alternative approaches}
% \label{sec:memory_management_alternatives}
% 
% There are several approaches that could be used to manage the CUDA memory space.
% %
% \begin{itemize}
% \item Reference counting can be used to count the number of times an array is
%     referenced within the GPU computation, and deallocate the array after the
%     final use. This approach is fragile and error, because it must be reassessed
%     each time the core language changes, but results in the shortest array
%     lifetimes.
% 
% \item A simpler alternative is a syntax directed approach. Since any array used
%     multiple times in an expression will be let-bound
%     (\S\ref{sec:sharing_observation}), the bound array can be deallocated after
%     evaluating the body of the let binding.
% 
% \end{itemize}
