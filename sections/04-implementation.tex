...
\marginnote{tk:figures}
The basic binary-tree based scan algorithm proceeds in $\log N$ stages. As
shown in Figure~\ref{fig:tree_scan}, the binary operator is applied to two
distinct elements of the input array and the output is stored into a new array.
In order to compute this efficiently on the GPU, the input array is decomposed
into a number of blocks of size $B$, such that the scan of $B$ elements can be
computed by a thread block locally in shared memory. This scan algorithm is
depicted in Figure~\ref{fig:fast_scan}, and proceeds in two phases:
%
\begin{enumerate}
\item All blocks are scanned in parallel, and the partial result from each block
    @i@ is stored into an intermediate array @block_results[i]@.

\item Perform a scan of the helper array @block_results@. Accelerate is set up
    such that the number of blocks in the first step is less than the maximum
    thread block size (\S\ref{sec:executing_programs}), so this step does not
    need to repeatedly apply the global scan algorithm.

\item Each thread of block @i@ adds element @i@ from Step 2 to its output
    element from Step 1.
\end{enumerate}
...
compare-and-swap in this manner. Tuple types apply the procedure to each
component separately, as CUDA currently only supports @atomicCAS@ style
instructions. Since Accelerate uses a struct-of-arrays style representation,
atomically committing all components of a tuple would require additional
hardware support. Utilising more specific atomic operations such as @atomicAdd@
when available is left to future work.
...
array. Boundary conditions determine how to handle out-of-bounds neighbours, by
specifying a constant value to use instead, or by reading the source array at a
different index.
% (clamping to the boundary; wrapping to the other side of array; or mirroring
% in from the edge the number of elements attempted beyond the boundary).
However, with the array sizes of typical GPU programs, the border region
represents only a tiny fraction of the entire array. Thus, for optimal
performance we want to avoid testing of the boundary each time we read an
element from the source array.
\marginnote{tk: picture? more info?}
...
parallel tree reduction~\cite{Chatterjee:2009vh}.
...
(SoA)\index{SoA|see {struct-of-array}} representation stores arrays of type
...
environment. This is the method used when pretty printing Accelerate
expressions.
...
multi-dimensional indices to the linear array representation. Since the source
...
the same parallel operation, rather than computing the result once and sharing
the result.}
...
\item Once the kernel is compiled, details such as the number of registers
    ...
    order to maximum \indexe{thread occupancy}, which is discussed further in
...
\note{TK: perhaps a graph of how occupancy calculations change for a given kernel on different GPU architectures?}
...
for nested parallelism, recursion, and sum data types.
