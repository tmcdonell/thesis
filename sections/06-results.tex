...
times are measured directly by compiling separately with the @nvcc@ command line
tool. Times are the average of 10 evaluations.
...
For matrices such as Dense, the increased memory bandwidth gained from aligning
global memory reads to the warp boundary offsets the additional startup cost,
and Accelerate exhibits higher performance than its competitors.
%
However, matrices such as the FEM/Spheres, which contain only a few non-zero
