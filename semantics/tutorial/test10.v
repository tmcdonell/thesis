...
(** substitutions *)
Fixpoint tsubt_t (t_6:t) (x5:termvar) (t__7:t) {struct t__7} : t :=
  match t__7 with
  | (t_Var x) => (if eq_termvar x x5 then t_6 else (t_Var x))
  | (t_Lam x t5) => t_Lam x (if list_mem eq_termvar x5 (cons x nil) then t5 else (tsubt_t t_6 x5 t5))
  | (t_App t5 t') => t_App (tsubt_t t_6 x5 t5) (tsubt_t t_6 x5 t')
end.
...
Inductive reduce : t -> t -> Prop :=    (* defn reduce *)
 | ax_app : forall (x:termvar) (t12 v2:t),
     is_v_of_t v2 ->
     reduce (t_App  (t_Lam x t12)  v2)  (tsubt_t  v2   x   t12 
